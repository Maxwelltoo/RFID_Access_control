...
/**
  * @brief          显示一个汉字
  * @param[in]      x: 字符的开始行 0~127
  * @param[in]      y: 字符的开始列 0~7
  * @param[in]      num: 字库序号
  * @retval         none
  */
void OLED_Show_CN(uint8_t x,uint8_t y,uint8_t num)
{
	uint8_t t,adder=0;
	OLED_set_pos(x,y);
	for(t=0;t<16;t++)
	{
		oled_write_byte(CN_char[2*num][t],OLED_DATA);
		adder+=1;
	}
	OLED_set_pos(x,y+1);
	for(t=0;t<16;t++)
	{
		oled_write_byte(CN_char[2*num+1][t],OLED_DATA);
		adder+=1;
	}
}
...
void OLED_Start(void)
{
	...
	for(i=0;i<6;i++)
	{
		OLED_Show_CN(16+16*i,0,i+1);
	}
	for(i=0;i<6;i++)
	{
		OLED_Show_CN(16+16*i,2,i+7);
	}
	for(i=0;i<6;i++)
	{
		OLED_Show_CN(16+16*i,4,i+18);
	}
	for(i=0;i<3;i++)
	{
		OLED_Show_CN(64+16*i,6,i+13);
	}
}
...
void OLED_Welcome(void)
{
	...
	for(i=0;i<4;i++)
	{
		OLED_Show_CN(32+16*i,2,i+24);
	}
	for(i=0;i<2;i++)
	{
		OLED_Show_CN(32+16*i,4,i+32);
	}
	OLED_Show_CN(64,4,38);
}
...
void OLED_Error(void)
{
	...
	for(i=0;i<4;i++)
	{
		OLED_Show_CN(32+16*i,2,i+28);
	}
	for(i=0;i<4;i++)
	{
		OLED_Show_CN(32+16*i,4,i+34);
	}
	OLED_Show_CN(64,4,38);
}
